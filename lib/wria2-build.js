...
function _war(options, done) {
  ...
  fullWar = [
    ...
    function (yui3path, callback) {
      ...
      var dstOut = (userChoices.remote) ? process.env.HOME : process.cwd(),
        cmdline =
    ...
    function (yui3path, callback) {
      ...
      if (userChoices.remote) {
        dstWar = path.join(process.env.HOME, war);
        // file was not moved, let's do it

        if (fs.existsSync(srcWar)) {
          utilities.copyThenEraseSync(srcWar, dstWar);
        } else if (fs.existsSync(srcWar2)) {
          utilities.copyThenEraseSync(srcWar2, dstWar);
        }
