...
var async = require('async'),
  ...
  servers = {},
  tmpBuildDirName = 'build_tmp',
  userChoices = {
    branch: '',

    tmpClonePath: path.join(tmpDir, tmpBuildDirName)
  };
...
function _gatherUserChoicesForWar(options, done) {
  async.waterfall([
      ...
      function (callback) {
        var msg = 'Type the branch you want to build, or ENTER for default [' +
          options.pkgConfig.defaultBranch + ']';
        utilities.promptAndContinue({
          promptType: utilities.PROMPT_PROMPT,
          promptMsg: msg,
          defaultValue: options.pkgConfig.defaultBranch
        }, function (err, value) {
          if (value) {
...
function _war(options, done) {
  ...
  fullWar = [
    ...
    function (callback) {
      var time = (process.platform === 'win32') ? 22 : 10;
    ...
    function (callback) {
      // need to create wf2_package/build directory (bypassing maven)
      mkdirp(path.join(userChoices.tmpClonePath, 'wf2_package', 'build'), callback);
    },
    function (callback) {
      // need to do a shallow clone of yui3
      yui3Utils.cloneTemporaryYui({
        branch: 'wf2-' + options.pkgConfig.defaultBranch,
    ...
    function (yui3path, callback) {
      // need to start the wf2 build
      var opt = (process.platform === 'win32') ?
        ' -c \'"../../../wria/combo?basePath=@WF2_VERSION@/build&"\'' :
        ' -c "../../../wria/combo?basePath=@WF2_VERSION@/build&"',
        cmdline = 'node build/lib/wf2_package.js' + opt +
          ' -l -i ' + userChoices.branch + '-' + userChoices.username +
          ' -o ' + path.join(userChoices.tmpClonePath, 'wf2_package');
    ...
    function (yui3path, callback) {
      ...
      log.echo('You may want to remove the cloned repos: ');
