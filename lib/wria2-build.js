...
function _gatherUserChoicesForWar(options, done) {
  async.waterfall([
      ...
      function (callback) {
        var msg = 'Run the build locally [L] or on Jenkins [J]:';
        utilities.promptAndContinue({
          promptType: utilities.PROMPT_PROMPT,
          promptMsg: msg,
          validator: function (value) {
            value = value.toLowerCase();
            if (value !== 'l' && value !== 'j') {
              log.error('Please choose [L]ocal or [j]enkins...');
              throw new Error();
            } else {
              return value;
            }
    ...
    function (callback) {
      if (userChoices.remote) {
        ...
        if (userChoices.buildLocation === 'j') {
          log.info('Running WAR build on Jenkins server...');
          var res, cmdline = 'ssh linux "source ~/.profile; fedtools war -u ' + userChoices.username +
