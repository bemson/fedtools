  ...
  fullWar = [
    ...
    function (callback) {
      var res, cmdline, time,
        cmdlineTpl = 'source ~/.bash_profile; ' +
      ...
      if (userChoices.remote) {
        ...
        if (userChoices.buildLocation === 'j') {
          ...
          log.info('Estimated time remaining: about 25 minutes');
          ...
          log.info('Estimated time remaining: about ' + time + ' minutes');
          utilities.promptAndContinue({
  ...
  async.waterfall([
    ...
    function (err, data) {
      if (err === REMOTE_EXEC_RETURN_CODE.SUCCESS) {
        ...
        log.success('WAR file built successfully!');
        log.info('Let\'s get it back via ssh!');
        log.notice('You may have to re-type the warbuild password...');
        log.echo();
        log.echo('WAR file: ', war);
        log.echo('WAR path: ' + process.cwd());
        log.echo();
