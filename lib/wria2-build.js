  ...
  fullWar = [
    ...
    function (callback) {
      var res, cmdline, cmdlineTpl = 'source ~/.bash_profile; ' +
          'fedtools war -r -u {{username}} -w {{wriaBranch}} -y {{yuiBranch}}';
    ...
    function (callback) {
      ...
      // We can also know what the WAR file will be
      srcWar = path.join(userChoices.tmpClonePath,
        'wt2', 'wria2-documentation', 'target', 'wria2-documentation.war');
      callback();
    },
    function (callback) {
  ...
  async.waterfall([
    ...
    function (err, data) {
      if (err === REMOTE_EXEC_RETURN_CODE.SUCCESS) {
        ...
        // we need to scp the file back here
        var client, dstWar, war, cmdline;

        war = userChoices.branch + '-' + userChoices.username + '.war';

        srcWar = path.join('/tmp/fedtools-tmp/build_tmp_' + userChoices.username,
          'wt2', 'wria2-documentation', 'target', 'wria2-documentation.war');
        dstWar = path.join(process.cwd(), war);
        cmdline = 'scp ' + pkgConfig.jenkinsusername + '@' + pkgConfig.jenkinshostname +
          ':' + srcWar + ' ' + dstWar;
