...
function _gatherUserChoicesForWar(options, done) {
  async.waterfall([
      ...
      function (callback) {
        var msg =
          'Do you want to [A]dd a job, [r]emove a job or check the job [s]tatus? [A|r|s]:';
        utilities.promptAndContinue({
          promptType: utilities.PROMPT_PROMPT,
          promptMsg: msg,
          validator: function (value) {
            value = value.toLowerCase();
            if (value !== 'a' && value !== 'r' && value !== 's') {
...
function _displaySummaryAndConfirmation(done) {
  ...
  if (userChoices.buildLocation === TYPE_WAR_BUILD_REMOTE) {
    ...
    if (process.platform !== 'win32') {
      log.notice('We know who you are >:)');
      log.notice('If possible, try to run it locally...');
    }
    log.echo();
  }
  utilities.promptAndContinue({
  ...
  if (options.remote && options.processJob) {
    ...
    if (fs.existsSync(warJobQueueFile)) {
      ...
      if (job) {
        ...
        if (utilities.sendSignal(job.pid, ALIVE_SIGNAL)) {
          ...
          // if it is, kill it!
          if (moment(job.startTime).add('seconds', 30) - moment() < 0) {
    ...
    jobs.forEach(function (job, index) {
      ...
      if (job.startTime) {
        ...
        log.echo(' Estimated completion time : ', moment(job.startTime).add('minutes', 25)
          .fromNow());
      }
    });
    done(null);
