...
var async = require('async'),
  ...
  warJobQueueFile,
  headerTpl = fs.readFileSync(path.join(__dirname, '..',
    'data', 'templates', 'war', 'header.mustache'), 'utf8'),
  footerTpl = fs.readFileSync(path.join(__dirname, '..',
...
function _gatherUserChoicesForWar(options, done) {
  async.waterfall([
      ...
      function (callback) {
        if (userChoices.queueRequest === REMOTE_ADD || userChoices.queueRequest ===
          ...
          var userbranch = utilities.fedtoolsRcGet(utilities.FEDTOOLSRCKEYS.userbranch),
            msg = 'Type the WFRIA2 branch you want to build';
          if (userbranch) {
      ...
      function (callback) {
        if (process.env.FEDTOOLS_PASSWORD && userChoices.buildLocation ===
          TYPE_WAR_BUILD_LOCAL) {
...
function _displaySummaryAndConfirmation(done) {
  if (userChoices.buildLocation === TYPE_WAR_BUILD_REMOTE && userChoices.queueRequest ===
    ...
    log.blue('User name                 : ', userChoices.username);
    if (userChoices.useremail) {
      log.blue('User email                : ', userChoices.useremail);
    }
    log.blue('WF-RIA2 branch            : ', userChoices.branch);
    log.blue('YUI3 branch               : ', userChoices.yuiBranch);
    if (process.env.FEDTOOLS_PASSWORD) {
      log.blue('Custom WAR id             : ', (userChoices.customWARid) ? 'yes' : 'no');
    }
    if (userChoices.buildLocation === TYPE_WAR_BUILD_REMOTE) {
      if (userChoices.queueRequest === REMOTE_ADD) {
        log.blue('Build location            : ', 'remote');
        log.blue('Estimated completion time : ', 'about 15 minutes');
      }
...
function _sendErrorEmailIfRemote(options, done) {
  var htmlBodytpl = fs.readFileSync(path.join(__dirname,
    '..', 'data', 'templates', 'war', 'error.mustache'), 'utf8');

  if (userChoices.remote) {
    utilities.sendEmail({
      to: userChoices.useremail + ', arno.versini@wellsfargo.com',
      subject: 'WAR remote build [ FAILED ]',
  ...
  fullWar = [
    ...
    function (callback) {
      ...
      gith.cloneGitRepository({
        ...
        var msg = 'Cannot clone WF-RIA2 branch ' +
          userChoices.branch + ' for fork ' + userChoices.username;
        if (!err) {
          callback(null, 'Repository cloned in temporary location...');
        } else {
          _sendErrorEmailIfRemote({
            msg: msg
          }, function () {
            callback(err, data);
          });
    ...
    function (callback) {
      ...
      yui3Utils.cloneTemporaryYui({
        ...
        var msg = 'Cannot clone YUI3 branch ' + userChoices.yuiBranch;
        if (!err) {
    ...
    function (yui3path, callback) {
      if (userChoices.customWARid) {
        callback(null, yui3path, 'custom');
      } else {
        gith.getCurrentSHA({
          cwd: userChoices.tmpClonePath,
          short: true
        }, function (err, sha) {
          callback(null, yui3path, sha);
        });
      }
    },
    function (yui3path, sha, callback) {
      // need to start the rest of the build with maven
      log.info('Running maven WAR build, time to grab a coffee...');

      var dstOut = (userChoices.remote) ? userChoices.tmpClonePath : process.cwd(),
        logFile = path.join(userChoices.tmpClonePath, userChoices.username + '.log'),
        cmdline =
          'node build/lib/package.js -c "../../../wria/combo?basePath=@WF2_VERSION@/build&"' +
          ' -l -i ' + sha + '-' + userChoices.branch + '-' + userChoices.username +
          ' -v ' + sha + ' -o ' + dstOut;
    ...
    function (yui3path, sha, callback) {
      var war, srcWar2, dstWar, htmlBodytpl, logFile, _logSuccess;

      war = sha + '-' + userChoices.branch + '-' + userChoices.username + '.war';
      srcWar = path.join(userChoices.tmpClonePath, 'wt2', 'wria2-documentation', 'target', war);
      srcWar2 = path.join(userChoices.tmpClonePath, 'wt2', 'wria2-documentation', 'target',
        'wria2-documentation.war');
      dstWar = path.join(process.cwd(), war);
      logFile = (userChoices.remote) ? path.join(userChoices.tmpClonePath, userChoices.username +
        '.log') : null;

      _logSuccess = function () {
      ...
      if (userChoices.remote) {
        dstWar = path.join(userChoices.tmpClonePath, war);
        // if file was not moved, let's do it
        if (fs.existsSync(srcWar)) {
          fs.renameSync(srcWar, dstWar);
        } else if (fs.existsSync(srcWar2)) {
          fs.renameSync(srcWar2, dstWar);
        }
        ...
        utilities.sendEmail({
          ...
          htmlBody: mustache.render(htmlBodytpl, {
            ...
            yuiBranch: userChoices.yuiBranch,
            war: war,
            elapsedTime: moment().from(moment(now), true),
            srcWar: dstWar,
            jenkinsusername: pkgConfig.jenkinsusername,
            jenkinshostname: pkgConfig.jenkinshostname
          }, {
...
function _addJobToQueueFile(file, jobs, options) {
  ...
  msg = msg + 'It means that your job should be done in about ' +
    parseInt(JOB_ETA_MINUTES * (len + 1), 10) +
...
function _processWarOptions(options, done) {
  var res, msg, index, job, htmlBodytpl, jobs = [];
  ...
  fullBuild = [
    ...
    function (callback) {
      var buildPath = path.join(options.srcPath, 'wf2'),
        cmdline = SHIFTER + ' --coverage false --lint false --csslint false';
