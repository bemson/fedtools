  ...
  fullWar = [
    ...
    function (callback) {
      var res, cmdline, cmdlineTpl = 'source ~/.bash_profile; ' +
          'fedtools war -r -u {{username}} -w {{wriaBranch}} -y {{yuiBranch}} {{queueRequest}}';
      ...
      if (userChoices.remote) {
        ...
        if (userChoices.buildLocation === TYPE_WAR_BUILD_REMOTE) {
          ...
          res = cmd.runRemote(cmdline, {
            ...
            var index, job, jobs = JSON.parse(fs.readFileSync(warJobQueueFile, 'utf8'));

            job = _.find(jobs, function (obj, key) {
              if (obj.username === userChoices.username &&
                obj.wriaBranch === userChoices.wriaBranch) {
                index = key;
                return true;
              }
            });
            ...
            if (code !== 0) {
              ...
              // need to scp the file back and need to bypass the rest
    ...
    function (yui3path, callback) {
      ...
      if (userChoices.remote) {
        ...
        htmlBodytpl = 'The WAR file has been built successfully.\n' +
          'The name of the file is {{war}}\n' +
          'It can be found on Jenkins at that location: {{srcWar}}\n' +
          '\n' +
          'You have to download it to your local machine so that you can upload it to the ABC server.\n' +
          'Here is the command line to download it via scp:\n' +
          'scp {{jenkinsusername}}@{{jenkinshostname}}:{{srcWar}} .\n' +
          '\n' +
          'Thanks!\n' +
          'The Fedtools team :)';
        ...
        utilities.sendEmail({
          ...
          subject: 'WAR build remote successful',
  ...
  async.waterfall([
    ...
    function (err, data) {
      if (err === REMOTE_EXEC_RETURN_CODE.SUCCESS) {
        ...
        // we are coming from a successful remote request
        // so we need to scp the file back here (only if not windows)
        // log.success('Remote WAR build is complete!');

        // war = 'custom-' + userChoices.branch + '-' + userChoices.username + '.war';
        // srcWar = path.join('/tmp/fedtools-tmp/build_tmp_' + userChoices.username,
        // 'wt2', 'wria2-documentation', 'target', war);
        // dstWar = path.join(process.cwd(), war);

        // if (process.platform === 'win32') {
        //   log.info('The WAR file is available on the Jenkins server');
        //   log.info('You can use "scp" to get it back there:');
        //   log.echo('scp ' + pkgConfig.jenkinsusername + '@' +
        //     pkgConfig.jenkinshostname + ':' + srcWar.replace(/\\/g, '/') + ' .');
        //   log.echo();
        //   done();
        // } else {
        //   log.info('Downloading WAR file locally...');
        //   utilities.downloadFileOverSSH({
        //     host: pkgConfig.jenkinshostname,
        //     username: pkgConfig.jenkinsusername,
        //     password: userChoices.jenkinsuserpassword,
        //     srcFile: srcWar,
        ...
        //   }, function (err) {
        //     if (err) {
        //       log.error('Unable to get the WAR file back! ');
        //       log.echo('You may want to look for it on the jenkins server?');
        //       log.echo('It should be at that location:');
        //       log.echo(srcWar);
        //     } else {
        //       log.success('WAR file downloaded successfully from Jenkins!');
        //       log.echo();
        //       log.echo('WAR file name  : ', war);
        //       log.echo('WAR local path : ' + process.cwd());
        //       log.echo();
...
function _addJobToQueueFile(file, jobs, options) {
  ...
  if (len > 0) {
    msg = len === 1 ? 'There is ' : 'There are ' + len +
      ' scheduled jobs before yours. It means that your job should be done in about ' +
      parseInt(JOB_TTL_MINUTES * (len + 1), 10) + ' minutes.';
  } else {
    msg = 'Your job is the first in the queue, it shoud be done in about ' +
      parseInt(JOB_TTL_MINUTES - 5, 10) + ' minutes.';
  }
...
function _processWarOptions(options, done) {
  var res, index, isAlive, signal, result, job, jobs = [],
    KILL_SIGNAL = 'SIGINT',
    ALIVE_SIGNAL = 0;

  if (options.remote && options.processJob) {
    console.log('==> need to run the oldest job if not one of them is running');
    if (fs.existsSync(warJobQueueFile)) {
      ...
      if (job) {
        console.log('==> job may be running: ', job);
        // check if it's still running...
        // if not remove the entry from the queue
        if (utilities.sendSignal(job.pid, ALIVE_SIGNAL)) {
          console.log('==> it is alive!');
          // need to check if it's too old
          // if it is, kill it!
          if (moment(job.startTime).add('minutes', JOB_TTL_MINUTES) - moment() < 0) {
            console.log('==> job is too old, killing it');
            utilities.sendSignal(job.pid, KILL_SIGNAL);
            // not cleaning the queue file to give the process to stop
        ...
        if (job) {
          console.log('==> need to run this job: ', job);
    ...
    console.log('==> need to add a job');
    console.log('==> mkdirp there: ', runDir);
    mkdirp.sync(runDir);
    if (fs.existsSync(warJobQueueFile)) {
      jobs = JSON.parse(fs.readFileSync(warJobQueueFile, 'utf8'));
      res = _.find(jobs, function (obj, key) {
    ...
    console.log('==> need to remove a job');
    if (fs.existsSync(warJobQueueFile)) {
      jobs = JSON.parse(fs.readFileSync(warJobQueueFile, 'utf8'));
