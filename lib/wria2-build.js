...
function _war(options, done) {
        ...
        if (userChoices.buildLocation === 'j') {
          ...
          if (res) {
            if (res.code !== 0) {
              ...
              // TODO need to scp the file back
              // and need to bypass the rest
              callback(-2);
            }
  ...
  async.waterfall([
    ...
    function (err, data) {
      if (err === -2) {
        var dstWar = path.join(process.cwd(), srcWar),
          war = packageVersion + '-' + userChoices.branch + '-' + userChoices.username +
            '.war',
