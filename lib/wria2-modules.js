...
var path = require('path'),
  ...
  cwd = process.cwd(),
  tplDir = path.join(__dirname, '..', 'data', 'templates', 'wria2', 'module'),
...
function _isFileUsedForSkin(file) {
  return ((path.extname(file) === '.scss') || (path.extname(file) === '.rb'));
}
...
function _buildFiles(dstPath, done) {
  ...
  files.forEach(function (file) {
    ...
    if (!(isJSModule && _isFileUsedForSkin(destFileName))) {
...
function _displaySummaryAndPromptToContinue(rootPath, done) {
  ...
  log.blue('Create a wt2 module : ', (userChoices.wt2module) ? 'yes' : 'false');
...
exports.run = function (done) {
  ...
  async.waterfall([
    ...
    function (callback) {
      _promptForInput({
        msg: 'Type the class name of the module you want to create, or ENTER for default [' +
          userChoices
          .className + ']: ',
    ...
    function (callback) {
      _promptForInput({
        msg: 'Type the base name of the module you want to create, or ENTER for default [' +
          userChoices
          .baseName + ']: ',
    ...
    function (callback) {
      _promptForInput({
        msg: 'Type the namespace for the module you want to create,' +
          ' or ENTER for default [' + userChoices
          .namespace + ']: ',
    ...
    if (!err) {
      ...
      utilities.parseTree(modulePath, rootPath, function (tree) {
        console.log('\n' + tree);
        if (userChoices.wt2module) {
          modulePath = path.join(rootPath, 'wf2', 'src',
            userChoices.moduleName.replace(/^wf2-/, 'wt2-'));
          utilities.parseTree(modulePath, rootPath, function (tree) {
            console.log('\n' + tree);
          });
        }
