...
var _runSync = function (cmds, options, callback) {
  ...
  for (i = 0; i < len; i += 1) {
    display = cmds[i].slice(0, log.MAX_STR_LEN) + '...';
    if (!silent) {
...
exports.run = function (cmdLine, options, callback) {
  var command, cmdArray, preDisplay, display, syncFlag = false;
  ...
  if (_.isArray(cmdLine) || !callback) {
    ...
    // cmdLine is a string of space separated strings and the first
    // one is the command. We need to shorthen the command for
    // display purposes: no need to see the full path
    cmdArray = cmdLine.split(' ');
    command = path.basename(cmdArray[0]);
    cmdArray.shift();
    preDisplay = command + ' ' + cmdArray.join(' ');
    display = preDisplay.slice(0, log.MAX_STR_LEN) + '...';
