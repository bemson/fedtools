...
var clc = require('cli-color'),
  ...
  boring = false,
  MAX_STR_LEN = 64,
  STATUS_POS = MAX_STR_LEN + 4, // '...' plus one space
  color = {
    'blue': (process.platform === 'win32') ? clc.cyan : clc.blue,
    'yellow': clc.yellow,
    'green': clc.green,
    'red': clc.red
  },
...
function _displayStatus(type) {
  process.stdout.write(clc.move(STATUS_POS, -1));
...
var _buildString = function (truncate, args) {
  ...
  if (truncate && outString.length > MAX_STR_LEN) {
    outString = outString.slice(0, MAX_STR_LEN) + '...';
  }
  return outString;
};
 ...
 * Calling this function will remove all color output.
 * All calls to clc.<color> will print out in default white.
 *
 * @method setBoring
 */
exports.setBoring = function () {
  boring = true;

  state.info.log = clc.white;
  state.notice.log = clc.white;
  state.warning.log = clc.white;
  state.success.log = clc.white;
  state.error.log = clc.white;
  state.fatal.log = clc.white;

  color.blue = clc.white;
  color.yellow = clc.white;
  color.green = clc.white;
  color.red = clc.white;
};
...
module.exports.blue = function () {
  console.log(color.blue(_buildString(false, arguments)));
};
/**
 * Prints a string on the command prompt.
 * The color of the string is red.
...
module.exports.red = function () {
  console.log(color.red(_buildString(false, arguments)));
};
/**
 * Prints a string on the command prompt.
 * The color of the string is yellow.
...
module.exports.yellow = function () {
  console.log(color.yellow(_buildString(false, arguments)));
};
/**
 * Prints a string on the command prompt.
 * The color of the string is green.
...
module.exports.green = function () {
  console.log(color.green(_buildString(false, arguments)));
};
/**
...
module.exports.title = function () {
  console.log(color.blue.underline(_buildString(false, arguments)));
};

exports.MAX_STR_LEN = MAX_STR_LEN;
