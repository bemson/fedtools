      ...
      function (yui3path, callback) {
        gith.checkoutBranch({
          ...
          if (!err) {
            ...
            callback(err, 'Unable to checkout branch wf2-' + userChoices.branch +
              ' from YUI3 repository...');
          }
        });
      },
      function (yui3path, callback) {
      ...
      if (bootstrapped) {
        ...
        if (userChoices.build) {
          if (!err) {
            ...
            log.echo();
            log.echo('You can start Selleck to view example pages with the following command:');
            docsPath = path.relative(cwd, path.join(dstWf2srcPath, 'wf2-common', 'docs'));
            if (process.platform === 'win32') {
              docsPath = docsPath.replace(/\\/g, '/');
            }
            log.echo(' selleck -s -p ' + docsPath);
          } else {
