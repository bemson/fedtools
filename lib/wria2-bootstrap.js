...
function _gatherDefaultBranch(callback) {
  utilities.promptAndContinue({
    promptType: utilities.PROMPT_PROMPT,
    promptMsg: 'Please type the name of the branch you want to checkout, or ENTER to use the default one [' +
      userChoices.branch + ']:',
    defaultValue: userChoices.branch
  }, callback);
}

function _gatherYuiOptions(done) {
...
function _gatherUserChoices(done) {
  async.waterfall([
    ...
    function (callback) {
      if (userChoices.clone === 'false') {
        _gatherYuiOptions(function (err, data) {
          console.log('==> err: ', err);
...
function _displayOptionsAndConfirmation(pkgConfig, callback) {
  ...
  if (userChoices.clone === 'true') {
    log.blue('Sync YUI3 with repo : yes');
    log.blue('wria2 git url       : ', userChoices.url);
    log.blue('wf2-yui3 git url    : ', pkgConfig.wria2yui3giturl);
  ...
  async.waterfall([
      ...
      function (yui3path, callback) {
        var srcPath = path.join(yui3path, 'src'),
          dstPath = path.join(userChoices.path, 'wf2', 'src');
