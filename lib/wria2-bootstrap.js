...
function _gatherGitlabId(pkgConfig, callback) {
  var msg = 'Please type the name of your Gitlab fork (usually your username):';
  utilities.promptAndContinue({
    promptType: utilities.PROMPT_PROMPT,
    promptMsg: msg,
  }, function (err, value) {
  ...
  async.waterfall([
      ...
      function (callback) {
        ...
        if (userChoices.build === 'true') {
          ...
          // user doesn't want to build but we still can bootstrap
          // the build npm packages dependencies
          var buildPath = path.join(dstWf2srcPath, '..', '..', 'build');
          build.installLocalNpmPackages(buildPath, callback);
        }
      }
    ],
    function (err, data) {
