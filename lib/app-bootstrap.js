...
function _buildFiles(type, dstPath, done) {
  var files, dotFiles, tpl, buffer, realName, rootPath,
...
function _gatherUserChoices(type, done) {
  ...
  if (type === TYPE_FLOW) {
    ...
    async.waterfall([
      ...
      function (callback) {
        var msg = 'What version of the WF-RIA2 tags do you want to use?';
        utilities.promptAndContinue({
          promptType: utilities.PROMPT_PROMPT,
          promptMsg: msg,
        }, function (err, value) {
    ...
    function (callback) {
      if (type === TYPE_APP) {
        ...
        _buildFiles(TYPE_APP, userChoices.rootPath, function (err) {
          // and and example flow
          if (!err) {
            userChoices.flowName = 'flow1';
    ...
    if (!err) {
      if (type === TYPE_APP) {
        ...
        log.echo('http://localhost:8080/' + userChoices.appName + '-' +
          userChoices.appVersion + '/flow/flow1/page/page1');
      } else if (type === TYPE_FLOW) {
