...
function _buildFiles(type, dstPath, done) {
  ...
  files.forEach(function (file) {
    ...
    buffer = mustache.render(tpl, {
      ...
      namespaceLower: userChoices.namespace.toLowerCase(),
...
function _gatherUserChoices(type, done) {
  ...
  if (type === TYPE_FLOW) {
    async.waterfall([
      ...
      function (callback) {
        var msg = 'Type the namespace to use: ';
        utilities.promptAndContinue({
          promptType: utilities.PROMPT_PROMPT,
          promptMsg: msg,
        }, function (err, value) {
          if (value) {
            userChoices.namespace = value.toUpperCase();
          }
          callback();
        });
      },
    ...
    function (callback) {
      if (type === TYPE_APP) {
        if (userChoices.installNpmPackages) {
          ...
          log.notice('Estimated remaining time: between 1 and 3 minutes...');
          var appPath = path.join(userChoices.rootPath, userChoices.appName);
    ...
    if (!err) {
      if (type === TYPE_APP) {
        ...
        log.echo('You can type \'cd ' + userChoices.appName +
          ' && mvn fedtools:watch\' and access your app at:');
        ...
        _extractAppInformation(path.join(userChoices.rootPath), function (err, name, version) {
          if (!err) {
            log.echo('Type \'mvn fedtools:watch\' and access your new flow at:');
